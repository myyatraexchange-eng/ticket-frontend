...
export default function FindTicket() {
  const { tickets, addTicket, removeTicket, fetchMyTickets } = useTickets();
  const { token } = useAuth();
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 container mx-auto flex flex-col items-center">
      <h1 className="text-3xl font-bold mb-6 text-center text-blue-700 uppercase">🎟 Find Tickets</h1>

      {/* Filters */}
      <div className="flex gap-2 md:gap-3 mb-6 flex-wrap justify-center w-full max-w-4xl">
        <input placeholder="From" value={fromFilter} onChange={e=>setFromFilter(e.target.value)} className="border p-2 rounded w-36 md:w-44 uppercase text-sm"/>
        <input placeholder="To" value={toFilter} onChange={e=>setToFilter(e.target.value)} className="border p-2 rounded w-36 md:w-44 uppercase text-sm"/>
      ...
      <div className="grid gap-4 w-full max-w-6xl grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map(t => (
          <div key={t._id} className="rounded-xl shadow-lg p-5 bg-white border border-gray-200 hover:shadow-2xl transition min-h-[280px] flex flex-col justify-between">
            <div className="flex flex-col gap-2 text-sm">
              <h2 className="text-xl font-semibold text-blue-700 mb-2 uppercase">🚆 {t.trainName?.toUpperCase() || "UNKNOWN"} ({t.trainNumber || "N/A"})</h2>
              <p className="uppercase"><span className="font-semibold">📍 Route:</span> {t.from?.toUpperCase()} → {t.to?.toUpperCase()}</p>
              ...
              <p className="uppercase"><span className="font-semibold">🪑 Class:</span> {t.classType?.toUpperCase()}</p>
              <p className="uppercase"><span className="font-semibold">🎟 Tickets:</span> {t.ticketNumber}</p>
              {!t.contactUnlocked && <button onClick={()=>handlePay(t)} className="mt-2 w-fit bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition uppercase text-sm">Pay ₹20 to Unlock Contact</button>}
            </div>

            {showQR && currentTicketId === t._id && currentUpiLink && (
