...
const FindTicket = () => {
  ...
  const fetchTickets = async (query = {}) => {
    try {
      ...
      const res = await axios.get(`${API_BASE}/ticket${params ? `?${params}` : ""}`);
  ...
  if (loading)
    return <p className="text-center mt-10 text-gray-500 animate-pulse">Loading tickets...</p>;

  return (
    <div className="max-w-7xl mx-auto p-4">
      {/* Page Heading */}
      <div className="flex items-center mb-6">
        <FaTicketAlt className="text-blue-600 mr-2 text-2xl" />
        <h1 className="text-2xl font-bold text-blue-600">Find Tickets</h1>
      </div>
              ...
              {!t.contactUnlocked && (
                <div className="mt-3 border-t pt-3">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInput(t._id, "txnId", e.target.value)}
                  />
                  <input
                    type="text"
                    placeholder="Payer Name"
                    className="w-full mb-2 border rounded px-2 py-1"
                    onChange={(e) => handleInput(t._id, "payerName", e.target.value)}
                  />
                  <input
                    type="text"
                    placeholder="Payer Mobile"
                    className="w-full mb-2 border rounded px-2 py-1"
                    onChange={(e) => handleInput(t._id, "payerMobile", e.target.value)}
                  />
                  <input
                    type="number"
                    placeholder="Amount"
                    className="w-full mb-2 border rounded px-2 py-1"
                    onChange={(e) => handleInput(t._id, "amount", e.target.value)}
                  />
                  <button
                    disabled={submitting}
