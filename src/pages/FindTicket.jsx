...
const FindTicket = () => {
  ...
  useEffect(() => {
    const filtered = tickets.filter(ticket => {
      ...
      const matchesFrom = fromFilter ? ticket.from === fromFilter : true;
      const matchesTo = toFilter ? ticket.to === toFilter : true;
  ...
  // Get unique stations for From/To dropdown
  const fromStations = [...new Set(tickets.map(t => t.from))];
  const toStations = [...new Set(tickets.map(t => t.to))];
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredTickets.map(ticket => (
            <div key={ticket._id} className="bg-white shadow-md rounded p-4 border hover:shadow-xl transition">
